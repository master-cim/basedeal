...
class Status(models.Model):
    ...
    description = models.TextField()
    prev_status = models.ForeignKey('self', null=True, blank=True,
                                    related_name='status',
...
class Order(models.Model):
    ...
    phone_regex = RegexValidator(regex=r'^\+?7?\d{10}$',
                                 message="Телефонный номер должен быть введен "
                                 "в формате: '+799999999'. "
                                 "Допускается не более 11 цифр.")
...
class Project(models.Model):
    ...
    order = models.ForeignKey('Order', on_delete=models.CASCADE,
                              ...
                              help_text='Выберите заказ', null=True,
                              blank=True,)
